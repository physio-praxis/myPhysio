...
class CookiesMock implements Pick<Cookies, 'get' | 'set'> {
	private map = new Map<string, string>();
	public setCalls: Array<{ name: string; value: string; opts: CookieSetOptions }> = [];

	get(name: string) {
		return this.map.get(name);
	}
	set(name: string, value: string, opts: CookieSetOptions) {
...
describe('session utils', () => {
	...
	describe('ensureAuthUser', () => {
		...
		it('throws when id is not a valid UUID', async () => {
			...
			// Act + Assert
			await expect(ensureAuthUser({ id: invalidId, email: 'fail@example.com' })).rejects.toThrow(
	...
	describe('createSessionForUserId', () => {
		...
		it('throws when user does not exist (foreign key violation)', async () => {
			...
			// Act + Assert
			await expect(createSessionForUserId(nonExistingUserId, 60)).rejects.toThrow(
	...
	describe('setSessionCookie', () => {
		it('sets cookie value and flags (secure=true when DEV=false)', () => {
			...
			// Act
			setSessionCookie(cookies as unknown as Cookies, session);
	...
	describe('clearSessionCookie', () => {
		it('empties value and sets epoch expiry', () => {
			...
			// Act
			clearSessionCookie(cookies as unknown as Cookies);
	...
	describe('readSession', () => {
		it('returns null when cookie is missing', async () => {
			...
			// Act
			const result = await readSession(cookies as unknown as Cookies);
		...
		it('returns null when DB has no matching row', async () => {
			// Arrange
			const cookies = new CookiesMock();
			cookies.set(SESSION_COOKIE_NAME, 'raw-token', { path: '/' });
		...
		it('returns null when session is expired', async () => {
			...
			const userId = randomUUID();
			const cookies = new CookiesMock();
			cookies.set(SESSION_COOKIE_NAME, token, { path: '/' });
		...
		it('returns null if expiresAt is now (expired)', async () => {
			...
			// Act
			const res = await readSession(cookies as unknown as Cookies);
	...
	describe('refreshSessionIfNeeded', () => {
		it('does no-op when time left is above threshold', async () => {
			...
			// Act
			const result = await refreshSessionIfNeeded(cookies as unknown as Cookies, session, 999_999);
		...
		it('extends expiry, updates DB and cookie when below threshold', async () => {
			...
			// Act
			const refreshed = await refreshSessionIfNeeded(cookies as unknown as Cookies, session, 1000);
		...
		it('updates DB & cookie when ttl is very small (<1s)', async () => {
			...
			// Act
			const refreshed = await refreshSessionIfNeeded(cookies as unknown as Cookies, session, 1);
